...
def ps1(apps, retraits=()):
    ...
    lines += [
        ...
        "$aFaire = @($apps | Where-Object { -not (Get-ItemProperty -Path $base -Name $_.marker -ErrorAction SilentlyContinue) })",
    ...
    lines += [
        ...
        "$moi = Join-Path $dir 'apps-install.ps1'",
        "$vue = Join-Path $dir 'apps-fenetre.ps1'",
        "try { Copy-Item -LiteralPath $PSCommandPath -Destination $moi -Force -ErrorAction Stop } catch { }",
        "",
        ...
        "",
        "try {",
        "  # 1. Le TRAVAIL, en SYSTEM, a chaque ouverture de session.",
        "  $a1 = New-ScheduledTaskAction -Execute 'powershell.exe' `",
        "        -Argument ('-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File \"' + $moi + '\"')",
