...
def ps1(url, token):
    ...
    L = [
        ...
        "",
        "# ── Diagnostic embarque ──────────────────────────────────────────────",
        "# Ecart d'horloge : mesure par l'en-tete « Date » de la passerelle, sans w32tm et sans",
        "# privileges. C'est la cause n°1 des strategies ORDINATEUR qui ne s'appliquent pas :",
        "# au-dela de 5 minutes, l'authentification est refusee et rien ne se deploie.",
        "$ecart = $null",
        "try {",
        "  $cb0 = [System.Net.ServicePointManager]::ServerCertificateValidationCallback",
        "  [System.Net.ServicePointManager]::ServerCertificateValidationCallback = { $true }",
        "  $t0 = Get-Date",
        "  $h = Invoke-WebRequest -Uri 'http://%s:2080/' -Method Head -UseBasicParsing -TimeoutSec 10" % '192.168.182.1',
        "  [System.Net.ServicePointManager]::ServerCertificateValidationCallback = $cb0",
        "  if ($h.Headers['Date']) { $ecart = [int]((New-TimeSpan -Start ([datetime]::Parse($h.Headers['Date'])).ToUniversalTime() -End $t0.ToUniversalTime()).TotalSeconds) }",
        "} catch { }",
        "",
        "# Journal des applications : dernieres lignes, pour comprendre un deploiement qui echoue",
        "# sans avoir a se deplacer sur le poste.",
        "$applog = ''",
        "try { if (Test-Path 'C:\\ProgramData\\Bastion\\apps.log') {",
        "  $applog = (Get-Content 'C:\\ProgramData\\Bastion\\apps.log' -Tail 25 -ErrorAction Stop) -join \"`n\" } } catch { }",
        "# Applications marquees comme installees par le deploiement.",
        "$appok = 0",
        "try { $appok = (Get-Item 'HKLM:\\Software\\Bastion\\Apps' -ErrorAction Stop).Property.Count } catch { }",
        "",
        "# Activation Windows : compte rendu depose par la strategie « Activation Windows/",
        "# Office ». Une montee d'edition qui echoue ne se voyait nulle part — il fallait",
        "# aller regarder le poste. Elle remonte desormais avec l'inventaire.",
        "$act = ''; $actd = ''",
        "try { $k = Get-ItemProperty 'HKLM:\\Software\\Bastion\\Activation' -ErrorAction Stop",
        "      $act = [string]$k.etat; $actd = [string]$k.detail } catch { }",
        "# Si la strategie n'est jamais passee, on lit tout de meme l'etat reel de la licence :",
        "# une case vide ne dit pas si le poste est active ou si le script n'a pas tourne.",
        "if (-not $act) {",
        "  try {",
        "    $l = Get-CimInstance SoftwareLicensingProduct -Filter \"ApplicationID='55c92734-d682-4d71-983e-d6ec3f16059f' AND PartialProductKey IS NOT NULL\" | Select-Object -First 1",
        "    if ($l -and $l.LicenseStatus -eq 1) { $act = 'hors-bastion'; $actd = 'active, mais pas par la strategie Bastion' }",
        "    else { $act = 'non-active'; $actd = 'strategie jamais appliquee' }",
        "  } catch { }",
        "}",
        "",
        "# Navigateur par defaut REELLEMENT en place, lu dans le choix de l'utilisateur.",
        "# La strategie d'associations ne s'applique qu'a la CREATION d'un profil : sans",
        "# cette remontee on croirait le parc bascule alors que seuls les nouveaux profils",
        "# le sont, et personne ne verrait la difference avant de s'en plaindre.",
        "$navig = ''",
        "try {",
        "  $navig = [string](Get-ItemProperty 'HKCU:\\Software\\Microsoft\\Windows\\Shell\\Associations\\UrlAssociations\\https\\UserChoice' -Name ProgId -ErrorAction Stop).ProgId",
        "} catch { }",
        "",
        "# Demarrage securise. ATTENTION : « try » comme VALEUR dans une table de hachage",
        "# multi-lignes n'est pas analysable par Windows PowerShell 5.1 — celui de tous les",
        "# postes. Le fichier entier etait alors rejete AVANT la premiere instruction : aucun",
        "# inventaire n'est jamais remonte, sans le moindre message. La valeur est donc",
        "# calculee AVANT la table. « -ErrorAction Stop » : sur un poste sans UEFI l'applet",
        "# signale une erreur non bloquante, que « catch » ne verrait pas autrement.",
        "$sb = -1",
        "try { $sb = [int](Confirm-SecureBootUEFI -ErrorAction Stop) } catch { $sb = -1 }",
        "",
        "$data=@{",
        "  horloge_ecart = $ecart",
        "  apps_log      = $applog",
