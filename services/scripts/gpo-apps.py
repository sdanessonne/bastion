...
def ps1(apps, retraits=(), generation=''):
    ...
    lines += [
        ...
        "    # On demande d abord la TAILLE annoncee par le serveur, pour pouvoir",
        "    # verifier ensuite qu on a bien tout recu.",
        "    #",
        "    # Sur une liaison Wi-Fi mediocre, le transfert se coupe en cours de route",
        "    # sans lever d erreur : Invoke-WebRequest rend la main, le fichier est la,",
        "    # mais incomplet. Vecu les 6 et 8 aout 2026 sur Firefox ESR — 70 Mo",
        "    # attendus, 51,9 puis 21,5 Mo recus. Sans ce controle on lance un installeur",
        "    # tronque, qui echoue de facon incomprehensible, et on recommence a chaque",
        "    # demarrage.",
        "    $attendu = 0",
        "    try { $attendu = [int64](Invoke-WebRequest -Uri $a.url -Method Head -UseBasicParsing -TimeoutSec 60).Headers['Content-Length'] } catch { }",
        "    Invoke-WebRequest -Uri $a.url -OutFile $tmp -UseBasicParsing -TimeoutSec 600 -ErrorAction Stop",
        "    $recu = (Get-Item $tmp).Length",
        "    if ($attendu -gt 0 -and $recu -lt $attendu) {",
        "      Set-ItemProperty -Path $base -Name ($a.marker + '_ko') -Value ($ko + 1)",
        "      Note ($a.marker + ' (' + $a.nom + ') : TELECHARGEMENT INCOMPLET — ' + [int]($recu/1MB) + ' Mo sur ' + [int]($attendu/1MB) + ' attendus (tentative ' + ($ko + 1) + '/' + $MAX_ECHECS + '). Lien reseau instable.')",
