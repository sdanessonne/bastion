...
def ps1():
    L = [
        ...
        "",
        "# ── 2. Demarrage rapide ──────────────────────────────────────────────────",
        "# Sans cela, Windows restaure l'etat du pave depuis la session precedente et la",
        "# valeur ci-dessus est ignoree un demarrage sur deux.",
        "$pw='HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Power'",
        "Set-ItemProperty -Path $pw -Name HiberbootEnabled -Value 0 -Type DWord",
        "$hb = (Get-ItemProperty -Path $pw -Name HiberbootEnabled).HiberbootEnabled",
        "Note ('demarrage rapide : HiberbootEnabled=' + $hb)",
        "",
        "# ── 3. Profils ───────────────────────────────────────────────────────────",
